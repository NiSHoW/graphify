...
- **Multiple repos, one database.** Without a namespace, two repos that share a database and a branch name (both have `main`) would delta-prune each other's graph on every update. `graphify backend set <uri> --project <name>` (or the `GRAPHIFY_NEO4J_PROJECT` env var, which wins) scopes every key to the project, so same-named branches coexist — useful on community edition, which has a single database. `graphify branches` lists (and `--prune` touches) only your own project's branches. Existing single-project databases are untouched: without `--project` the storage keys are byte-identical to before.
...
- **Fail closed.** If the backend is configured but unreachable, `build`/`update` abort instead of silently forking a file-only graph. If a push fails (or is killed) after a successful local rebuild, the local graph stays valid, the state is marked dirty, and the next run re-pushes the backlog.
...
Configuration: `graphify backend set` (writes `backend.json`, no credentials) or the `GRAPHIFY_NEO4J_URI` / `GRAPHIFY_NEO4J_USER` / `GRAPHIFY_NEO4J_DATABASE` / `GRAPHIFY_NEO4J_PROJECT` env vars; the password comes only from `GRAPHIFY_NEO4J_PASSWORD` (or the generic `NEO4J_PASSWORD` as fallback).
